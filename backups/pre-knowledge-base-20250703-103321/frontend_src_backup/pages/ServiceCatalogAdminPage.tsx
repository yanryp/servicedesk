...
const ServiceCatalogAdminPage: React.FC = () => {
  const { user } = useAuth();
  const [activeTab, setActiveTab] = useState<'overview' | 'catalogs' | 'items' | 'templates' | 'fields' | 'import'>('overview');
  ...
  const handleTabChange = (tab: typeof activeTab) => {
    ...
    if (tab !== 'items' && tab !== 'templates') setSelectedCatalog(null);
    if (tab !== 'templates' && tab !== 'fields') setSelectedServiceItem(null);
  };
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8">
              {[
                ...
                { key: 'templates', label: 'Service Templates', icon: DocumentTextIcon },
                { key: 'fields', label: 'Custom Fields', icon: CogIcon },
