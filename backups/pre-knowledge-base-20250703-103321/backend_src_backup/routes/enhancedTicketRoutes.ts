...
router.post('/create', protect, upload.array('attachments', 5), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Calculate SLA using advanced policy system
    const slaResult = await slaPolicyService.calculateSLAForTicket({
      priority: (priority as string) || 'medium',
      departmentId: serviceItem.serviceCatalog.departmentId,
      serviceCatalogId: serviceItem.serviceCatalogId,
      serviceItemId: serviceItem.id,
      createdByUserId: user!.id,
    ...
    const slaDueDate = slaResult.dueDate;
...
router.post('/bsg-tickets', protect, upload.array('attachments', 5), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Calculate SLA using advanced policy system
    const userDepartmentId = await slaPolicyService.getUserDepartment(user!.id);
    const slaResult = await slaPolicyService.calculateSLAForTicket({
      priority: priority || 'medium',
      departmentId: userDepartmentId,
      createdByUserId: user!.id,
      isKasdaTicket: bsgTemplate.category.name.includes('KASDA') || bsgTemplate.category.name.includes('Government'),
