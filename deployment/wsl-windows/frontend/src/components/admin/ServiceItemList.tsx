...
const ServiceItemList: React.FC<ServiceItemListProps> = ({ 
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/service-catalog-admin/items/${showDeleteConfirm.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
...
const ServiceItemEditor: React.FC<ServiceItemEditorProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing 
        ? `/api/service-catalog-admin/items/${item.id}`
        : `/api/service-catalog-admin/catalogs/${catalogId}/items`;
